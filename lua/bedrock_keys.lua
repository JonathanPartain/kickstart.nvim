...
function M.set_bedrock_keys(profile)
  profile = profile or 'dev-devaccess'
  local credentials = vim.system({ '/usr/local/bin/aws', 'configure', 'export-credentials', '--profile', profile }, { text = true }):wait()
  if credentials.code ~= 0 then
    vim.notify(vim.trim(credentials.stderr), vim.log.levels.ERROR)
    vim.notify(string.format('Make sure to log in to aws with the %s profile', profile), vim.log.levels.ERROR)
    return
  end
  ...
  local region_text = 'eu-central-1' -- hardcoded region
